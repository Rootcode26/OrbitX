...
export function RiskDistributionChart({ metrics }: { metrics: RiskDistributionMetric[] }) {
  return (
    <div className="px-4 py-4">
      <div className="chart-grow-x flex h-3 overflow-hidden">
        {metrics.map((metric) => (
      ...
      <div className="mt-3 divide-y divide-[var(--bd2)] border-t border-[var(--bd2)]">
        {metrics.map((metric, index) => (
          <div key={metric.level} className="chart-reveal grid grid-cols-[minmax(0,1fr)_80px_28px] items-center py-2" style={{ animationDelay: `${100 + index * 45}ms` }}>
