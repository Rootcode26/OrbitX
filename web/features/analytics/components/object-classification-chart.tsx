  ...
  return (
    <div className="space-y-3 px-4 py-4">
      {categories.map((category, index) => (
        <div key={category.label} className="chart-reveal" style={{ animationDelay: `${index * 45}ms` }}>
          ...
          <div className="mt-1.5 h-1.5 bg-[rgba(228,222,208,.035)]">
            <div className={`chart-grow-x h-full ${tones[category.tone]}`} style={{ width: `${(category.count / maximum) * 100}%`, animationDelay: `${70 + index * 45}ms` }} />
          </div>
        </div>
      ))}
    </div>
