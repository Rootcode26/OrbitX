    ...
    <div className="px-4 py-4">
      ...
      <div className="mt-3 grid h-[190px] items-end gap-2" style={{ gridTemplateColumns: `repeat(${Math.max(buckets.length, 1)}, minmax(0, 1fr))` }}>
        {buckets.map((bucket, index) => (
          <div key={bucket.label} className="chart-reveal flex h-full min-w-0 flex-col justify-end" style={{ animationDelay: `${index * 35}ms` }}>
            <div className="numeric mb-1 text-center text-[10px] text-text-secondary">{bucket.total}</div>
            <div className="chart-grow-y flex w-full flex-col justify-end" style={{ height: `${Math.max((bucket.total / maximum) * 145, 3)}px`, animationDelay: `${80 + index * 35}ms` }}>
