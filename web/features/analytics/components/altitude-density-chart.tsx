  ...
  return (
    <div className="space-y-1.5 px-4 py-4">
      {buckets.map((bucket, index) => (
        <div key={bucket.label} className="chart-reveal grid grid-cols-[72px_minmax(0,1fr)_28px] items-center gap-2" style={{ animationDelay: `${index * 25}ms` }}>
          ...
          <div className="h-2.5 bg-[rgba(228,222,208,.035)]">
            <div
              className={`chart-grow-x h-full ${bucket.highlighted ? "bg-high" : "bg-accent"}`}
              style={{ width: `${(bucket.count / maximum) * 100}%`, animationDelay: `${60 + index * 25}ms` }}
            />
          </div>
